...
export function useCreateEvent() {
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      if (data.max_attendees) fd.append('max_attendees', String(data.max_attendees))
...
export function useRsvpEvent() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (eventId: number) => api.post<EventRegistration>(`/events/${eventId}/rsvp`),
    onSuccess: () => {
...
export function useCancelRsvp() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (eventId: number) => api.delete(`/events/${eventId}/rsvp`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['events'] })
      qc.invalidateQueries({ queryKey: ['event-rsvps'] })
    },
